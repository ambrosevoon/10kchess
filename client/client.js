...
let interpolatingPieces = {/*squareX: {squareY: [curX, curY]}*/};
...
let camera = {x: -boardW * squareSize / 2, y: -boardH * squareSize / 2, scale: 1};
...
const tintedImgs = {/*hex: [array of images]*/}
...
function render() {
    ...
    bottomRight.x = Math.max(0,Math.min(boardW,Math.ceil(bottomRight.x / squareSize)));
    bottomRight.y = Math.max(0,Math.min(boardH,Math.ceil(bottomRight.y / squareSize)));
    ...
    if(legalMoves) {
        ...
        ctx.fillStyle = 'black';
        for(let i = 0; i < legalMoves.length; i++){
            if(legalMoves[i][0] < topLeft.x || legalMoves[i][0] > bottomRight.x || legalMoves[i][1] < topLeft.y || legalMoves[i][1] > bottomRight.y) continue;
        ...
        for(let j = topLeft.y; j < bottomRight.y; j++){
            if(board[i][j] !== 0){
                if(teams[i][j] === 0){
                    ...
                    if(interpolatingPieces[i] !== undefined && interpolatingPieces[i][j] !== undefined){
                        ...
                        if(Math.abs(interp[0] - i) < 0.01 && Math.abs(interp[1] - j) < 0.01){
    ...
    if(selectedSquareX !== undefined && draggingSelected === true){
        ...
        const img = tintedImgs[color.r + '_' + color.g + '_' + color.b][board[i][j]];
        ctx.drawImage(img, mousePos.x - squareSize / 2, mousePos.y - squareSize / 2);
    }
    ...
    if(mouse) {
        ...
        if(board[squareX] && board[squareX][squareY] !== 0 && teams[squareX][squareY] === selfId){
